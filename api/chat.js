export default {
    async fetch(request, env, ctx) {
        // Cloudflare Workers biasanya menangani routing berdasarkan method di sini
        ...
        try {
            // 1. Ambil API Key dari env Cloudflare
            // Pastikan Anda sudah setting GROQ_API_KEY di dashboard Cloudflare atau wrangler.toml
            const apiKey = env.GROQ_API_KEY; 

            if (!apiKey) {
                return new Response(JSON.stringify({ error: 'Missing API Key in environment variables.' }), {
                    status: 500,
                    headers: { 'Content-Type': 'application/json' }
                });
            }

            // 2. Extract message dan history dari request body (Standard Web API)

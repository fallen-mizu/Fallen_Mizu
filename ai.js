...
style.innerHTML = `
    ...
    .status-container { padding: 10px 15px; display: flex; align-items: center; justify-content: space-between; background: #fff; }
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        // Add Status Indicator above chat box
        if (chatBox && !document.getElementById('mizu-status')) {
            ...
            statusWrapper.innerHTML = `
                ...
                <div style="font-size: 10px; color: #999;">SYSTEM v3.1</div>
            `;
...
function updateMizuStatus(isOnline) {
...
window.sendMessage = async () => {
    ...
    try {
        ...
        document.getElementById(loadId)?.remove();
        updateMizuStatus(false);
        renderRow('mizu', "Connection lost or API limit reached. Please check back later.");
    }
};

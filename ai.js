...
style.innerHTML = `
    #main-content { max-width: 600px; margin: 0 auto; border: 1px solid #ddd; position: relative; background: #fff; }
    #chat-header { background: #2c3e50; color: white; padding: 15px; display: flex; align-items: center; justify-content: space-between; font-family: sans-serif; }
    .header-info { display: flex; flex-direction: column; }
    .status-indicator { font-size: 11px; display: flex; align-items: center; gap: 5px; margin-top: 2px; }
    .dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; }
    .online-dot { background: #2ecc71; box-shadow: 0 0 5px #2ecc71; }
    .offline-dot { background: #e74c3c; }

    #chat-box { display: flex; flex-direction: column; padding: 15px; gap: 15px; overflow-y: auto; height: 500px; scroll-behavior: smooth; background: #f5f6f7; }
    .chat-row { display: flex; width: 100%; margin-bottom: 5px; }
    .mizu-row { justify-content: flex-start; }
    .user-row { justify-content: flex-end; }
    
    .bubble { padding: 12px 18px; max-width: 80%; font-size: 14px; border-radius: 18px; line-height: 1.6; position: relative; box-shadow: 0 1px 2px rgba(0,0,0,0.1); }
    .mizu-bubble { background: #ffffff; color: #222; border-top-left-radius: 4px; }
    .user-bubble { background: #06c755; color: #fff; border-top-right-radius: 4px; } /* LINE Green */
    ...
    .typing-cursor { display: inline-block; width: 7px; height: 15px; background: #06c755; margin-left: 5px; animation: blink 0.8s infinite; vertical-align: middle; }
    ...
    .limit-banner { background: #fee2e2; color: #b91c1c; padding: 10px; border-radius: 8px; text-align: center; font-weight: bold; border: 1px solid #f87171; font-size: 12px; margin-bottom: 10px; }
`;
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        createHeaderUI(); // Panggil pembuatan header
...
function createHeaderUI() {
    const mainContent = document.getElementById('main-content');
    if (document.getElementById('chat-header')) return;

    const header = document.createElement('div');
    header.id = 'chat-header';
    header.innerHTML = `
        <div class="header-info">
            <span style="font-weight: bold; font-size: 16px;">Mizu</span>
            <div class="status-indicator" id="mizu-status">
                <span class="dot online-dot"></span>
                <span>Online</span>
            </div>
        </div>
        <div style="font-size: 12px; opacity: 0.8;">Architectural AI</div>
    `;
    mainContent.prepend(header);
}

function updateStatus(isOnline) {
    const statusDiv = document.getElementById('mizu-status');
    if (!statusDiv) return;
    if (isOnline) {
        statusDiv.innerHTML = `<span class="dot online-dot"></span><span>Online</span>`;
    } else {
        statusDiv.innerHTML = `<span class="dot offline-dot"></span><span>Offline</span>`;
    }
}
...
function applyLock() {
    ...
    updateStatus(false); // Mizu Offline
    ...
    if (input) { input.disabled = true; input.placeholder = "Mizu is offline..."; }
}
...
window.sendMessage = async () => {
    ...
    try {
        ...
        if ((snap.data().usageCount + 1) >= DAILY_LIMIT && !snap.data().isPremium) {
            applyLock();
        }
    } catch (err) {
